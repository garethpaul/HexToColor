...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="hextocolor-make-space-") as temporary:
        root = Path(temporary)
        copied = root / "repository with spaces"
        caller = root / "external caller"
        shutil.copytree(ROOT, copied, ignore=shutil.ignore_patterns(".git", "__pycache__", "*.pyc", "*.pyo"))
        caller.mkdir()
